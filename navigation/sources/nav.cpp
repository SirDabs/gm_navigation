...
void LUA_PushVector(GarrysMod::Lua::ILuaBase* L, const Vector& vec)
{
    L->GetField( GarrysMod::Lua::INDEX_GLOBAL, "Vector" );
    if( !L->IsType( -1, GarrysMod::Lua::Type::FUNCTION ) )
    {
        L->Pop( 1 );
        return;
    }
...
const Vector& LUA_GetVector(GarrysMod::Lua::ILuaBase* L, int index)
{
    L->CheckType(index, Type::VECTOR);
    UserData *udata = static_cast<UserData*>(L->GetUserdata(index));
    return *static_cast<const Vector*>(udata->data);
}

Nav* LUA_GetNav(GarrysMod::Lua::ILuaBase* L, int pos)
{
    L->CheckType(pos, NAV_TYPE);
    UserData *pUserData = static_cast<UserData*>(L->GetUserdata(pos));
	if(pUserData == NULL)
	{
		L->ArgError(pos, "Invalid Nav");
    }
    
    Nav *nav = reinterpret_cast<Nav*>(pUserData->data);
    if(nav == nullptr)
    {
        L->ArgError(pos, "Invalid Nav");
    }
...
void LUA_PushNav(GarrysMod::Lua::ILuaBase* L, Nav *nav)
{
	if(nav)
    {
        ...
        L->CreateMetaTableType( NAV_NAME, NAV_TYPE );
        L->SetMetaTable(-2);
	}
	else
	{
...
inline void UTIL_TraceLine_GMOD(const Vector& vecAbsStart, const Vector& vecAbsEnd, unsigned int mask, trace_t *ptr)
{
	...
	engine_trace->TraceRay(ray, mask, &traceFilter, ptr);
}
...
float Nav::HeuristicDistance(int heuristic, const Vector *vecStartPos, const Vector *EndPos)
{
...
{
	return (fabs(EndPos->x - vecStartPos->x) + fabs(EndPos->y - vecStartPos->y) + fabs(EndPos->z - vecStartPos->z));
}
...
void Nav::ExecuteFindPath(JobInfo_t *info, Node *start, Node *end)
{
	...
	AddOpenedNode(start);
	start->SetStatus(NULL, HeuristicDistance(info->heuristic, start->GetPosition(), end->GetPosition()), 0);

	float currentScoreG, scoreG;
	Node *current = NULL, *connection = NULL;
	int directions = info->diagonal ? NUM_DIRECTIONS_DIAGONAL : NUM_DIRECTIONS;
	...
	while(!info->abort)
	{
		...
		else
		{
			...
			for(int Dir = NORTH; Dir < directions; Dir++) // numDirections
			{
				...
				if(!connection->IsOpened() || scoreG < connection->GetScoreG())
				{
					...
					connection->SetStatus(current, scoreG + HeuristicDistance(info->heuristic, connection->GetPosition(), end->GetPosition()), scoreG);
				}
			}
		}
	}
